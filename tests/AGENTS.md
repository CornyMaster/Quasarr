...
- `test_crypter_cooldowns.py` pins the canonical protected-crypter resolver, import isolation, fixed three-distinct-observations/15-minute evidence rule, immutable duplicate evidence, first-surviving observation semantics, minimum cooldown, additive/malformed persistence handling, sanitized warnings, read-only snapshots, no-URL schema, state validation, and successful-access clearing. `test_sqlite_database.py` separately proves real-SQLite read/write traces, deterministic concurrent mutation ordering, begin/callback/type rollback, committed delete/upsert semantics, and nested Config/DataBase rejection.
