...
- `test_crypter_cooldowns.py` pins the canonical protected-crypter resolver, import isolation, fixed three-distinct-observations/15-minute evidence rule, immutable duplicate evidence and provisional retry deadlines, first-surviving observation semantics, minimum cooldown, additive/malformed persistence handling across every service API, sanitized warnings, read-only snapshots, no-URL schema, state validation, and successful-access clearing. `test_deferred_protected_packages.py` pins the protected-package `deferred` schema, field preservation, the single provisional hold budget, read-only defer reads, the dynamic queue projection and status prefix, and database-only batch deletion including per-ID verification and zero JDownloader calls. `test_sqlite_database.py` separately proves real-SQLite read/write traces, deterministic concurrent mutation ordering, begin/callback/type rollback, committed delete/upsert semantics, and nested Config/DataBase rejection.
