...
- Search-facing provider request helpers resolve `checkpoint()` / `clamp_timeout()` lazily inside their functions so providers do not create a module-import cycle back through `quasarr.search`. Cloudflare plain requests, solves, retries, and session creation; IMDb HTML/fallback requests; and Radarr/Sonarr requests all refuse a spent worker budget and clamp at the final network boundary. `flaresolverr_destroy_session` is deliberately not gated: destroying an already-created browser is required cleanup after exhaustion and keeps its existing session timeout.
