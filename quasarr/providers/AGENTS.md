...
- `crypter_cooldowns.py` - shared linkcrypter cooldown state machine; validates canonical protected-crypter keys, atomically records independent evidence, and manages one-time package probe flags
...
- Linkcrypter cooldown rows allow only `state="observing"|"cooldown"` and `reason_code="ip_block_suspected"`. Three distinct package IDs with three distinct SHA-256 link fingerprints inside the fixed 15-minute window create cooldown for at least the configured duration (minimum 24 hours); duplicate evidence only refreshes time, expired evidence is pruned, and successful access deletes the row. Rows persist package IDs and fingerprints but never raw URLs. One-time probe state is only `deferred.probe_requested` in the existing protected-package JSON, never a crypter state.
