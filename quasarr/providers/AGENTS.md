...
- `crypter_cooldowns.py` - shared linkcrypter cooldown state machine; validates canonical protected-crypter keys, atomically records independent evidence, owns protected-package defer metadata, and exposes read-only availability snapshots
...
- DB tables owned here: `sessions`, `hostname_issues`, `imdb_metadata`, `imdb_searches`, `xem_all_names`, `xem_season_names`, `statistics`, `crypter_cooldowns`.
...
- Package defer metadata lives only in the protected-package JSON under `deferred` and holds exactly `crypter`, `reason_code`, `since_epoch`, `retry_after_epoch`, `probe_requested`, and `observation_holds`. `defer_package()`/`clear_package_defer()` mutate it through one `mutate_value()` transaction that preserves every other package field and never creates a missing package; `get_package_defer()` is read-only and treats malformed metadata as absent after a sanitized warning. `observation_holds` is the provisional hold budget: a package receives at most one provisional hold and keeps that marker until it succeeds or is deleted, while a confirmed crypter cooldown (`observation_holds=0`) may still extend `retry_after_epoch`. `project_package_defer()` merges stored metadata with a live snapshot into `state`, `evidence_count`, the effective `retry_after_epoch`, and `hold_type` (`crypter_cooldown`/`provisional`/`none`); it never writes.
- Cooldown decoding validates every required record/observation field, accepts additive fields by projecting only known safe fields, and treats malformed rows as available after a concise warning containing only the normalized crypter key. Missing and valid unexpired `snapshot`/`is_cooling`/`retry_after` reads use `retrieve()` only; malformed or expired cleanup uses one `mutate_value()` transaction that revalidates the current value, preserves and returns a concurrent valid replacement, and deletes only a currently invalid/expired row.
- `crypter_cooldown_hours` is an optional shared-state dependency until Task 5 adds the setting. Missing/invalid values use 24 hours and lower values are clamped to 24; do not add settings-schema validation in this task.
