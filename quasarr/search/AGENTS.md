...
- Only an ordinary, complete release list that finished at or before the deadline is cached. `_is_cacheable(future, completion, deadline)` is the single gate and `collect()` is the only cache writer. Each submitted callable stamps `_Completion.at` in its `finally`, i.e. *before* its future is marked done - `Future.add_done_callback` cannot be used for it, because `set_result` wakes the `as_completed` waiter before it invokes callbacks, so the collecting thread could reach the cache write while the timestamp is still missing. `as_completed` keeps a 0.1s floor under its timeout, so a result landing just past the deadline is still collected: it is answered with and counted as `completed`, but never cached. Exceptions are excluded by `collect()`'s own `except` branch. A source returns a bare release list today, so a partial answer cannot be told apart from a complete one; Task 5 wraps the return value in a `SourceTaskOutcome` and adds `and not outcome.budget_exhausted` to this predicate - that is the only place budget-exhausted results need to be rejected.
