...
- `DataBase.mutate_values(targets, mutator)` is the same contract for several rows at once: `targets` are unique `(table, key)` pairs, missing tables are created first, the callback receives the current values in that order and returns exactly one replacement per target. Every table of a Quasarr database lives in one file, so one transaction on one connection commits all of them or none. Use it when a decision and the bookkeeping that must survive with it would otherwise be two commits with a crash window in between; do not use it to avoid one plain `mutate_value()`.
