...
- `submit_final_download_urls()` is the ONLY path that hands direct links to JDownloader (also imported by `api/captcha` and `api/sponsors_helper`). It applies the download-category mirror whitelist through the side-effect-free `project_final_download_urls()`; if a whitelist drops everything, the package is persisted as failed. Its default `phase="all"` keeps every existing caller's full submission/cleanup semantics, while `phase="submit"` stops immediately after a successful JDownloader handoff so the terminal route can durably mark `submitted` before cleanup.
- `jdownloader_holds_package()` is the crash-window reconciliation check for a resumed prepared terminal download. It searches the exact Quasarr package ID in the `comment` field of linkgrabber packages/links and downloader packages/links, in that order; `True` proves submission, `False` permits a retry after a proven failed handoff, and `None` means device state is unavailable and never permits another `addLinks`. `finalize_protected_removal()` deletes then reads back the protected row and reports both confirmed absence and whether this call performed the removal, which gates solved notification and counters exactly once. A failed-history cleanup uses the same proof with solved notification disabled.
