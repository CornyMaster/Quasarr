...
- **DL** — anchor and plaintext URLs are extracted from forum posts (crypter URLs — filecrypt/hide/keeplinks/tolink — plus whitelisted direct hosters; the plaintext scan skips text inside anchors to avoid duplicates); posts are iterated until one yields links verified online via their status URLs, keeping the first unverifiable post as fallback; fragment-pinned source URLs inspect only the identified post and return no links if that post is absent instead of widening to the whole thread; when the post names no password, a `www.<host>` default is used.
...
- **FF** — movie-only companion to the search source. `/external` URLs use manual redirect hops (`allow_redirects=False`, max 8 hops, loop/404 detection) and return as soon as a crypter is detected or the redirect leaves the configured FF host, so the plain path does not probe hoster/crypter pages. Release-page, movie-token API, and external-redirect requests use the shared Cloudflare host gate: the first blocked response enables direct FlareSolverr requests for that netloc for 24 hours. One lazily created browser session is reused for the whole grab, follows blocked external redirects, supplies their final URLs, and is always destroyed afterward. The requested release title is matched exactly, and the first requested mirror is resolved.
- **FX** — no module here; its filecrypt links flow through the orchestrator's crypter fallback into the protected `/captcha` userscript flow.
...
- **SF** — two URL modes: `/external` URLs use manual redirect hops (`allow_redirects=False`, max 8 hops, loop/404 detection) and stop as soon as `detect_crypter_type()` recognizes the URL, so the plain path does not load the crypter page. Series-page, season-API, and external-redirect requests use the shared Cloudflare host gate: the first blocked response enables direct FlareSolverr requests for that netloc for 24 hours. One lazily created browser session is reused for the whole grab, follows blocked external redirects, supplies their final URLs, and is always destroyed afterward. Series pages are matched via a strict release-title regex against the season API.
