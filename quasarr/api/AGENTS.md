...
- SponsorsHelper routes use trailing slashes; most require an active helper (HTTP 402 otherwise), tracked via `helper_last_seen` with a 300s timeout; `POST to_decrypt/` requires the helper's `supported_urls` and only hands out packages matching its URL patterns or optional `supported_mirrors`. Helpers advertising `crypter_defer_v1` skip only supported links under an active crypter cooldown or matching package hold, retain alternate crypters and later packages, may exclude at most 100 valid package IDs, and atomically consume a queued probe before handoff. Requests without that capability use the exact legacy selector path and ignore cooldowns, package holds, exclusions, and probes.
