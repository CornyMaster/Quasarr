...
- Authenticated `GET`/`POST /api/crypter-block/settings` refreshes/saves the linkcrypter block mode and cooldown hours through `storage.setup.crypter_blocks`. The existing Link Protection section server-renders those cached values and saves only `mode` plus `cooldown_hours` through `quasarrApiFetch`; it renders no linkcrypter URL or secret field.
- `POST /sponsors_helper/api/defer/` reads cached `crypter_block_mode` (`defer` when absent). `defer` records only validated BLOCKED evidence and returns exactly `hold`, `cooldown`, or `legacy_failure`; `fail` returns the exact available-state `legacy_failure` instruction before constructing or consulting cooldown/package-defer state. Neither mode calls the legacy failure path, deletes protected data, increments failed statistics, or emits FAILED notifications. In `defer`, a package receives at most one provisional hold, while confirmed cooldown always returns the crypter-wide deadline.
