...
- `POST /sponsors_helper/api/defer/` records only validated BLOCKED evidence and returns exactly `hold`, `cooldown`, or `legacy_failure`; it never calls the legacy failure path, deletes protected data, increments failed statistics, or emits FAILED notifications. A package receives at most one provisional hold, while confirmed cooldown always returns the crypter-wide deadline.
- `POST /sponsors_helper/api/crypter-access/` accepts only lowercase `clear`, validates the canonical package ID and protected crypter key, and requires the live package to contain a link resolving to that key before clearing both crypter and package defer state. This route is independent of later CAPTCHA, extraction, or download outcomes; UNKNOWN has no route call. A successful `/download/` request may additionally carry a top-level `crypter`; it clears that crypter only after final link submission succeeds, and a clear-persistence error cannot overturn the successful download response. Omitting the field preserves the legacy payload and behavior.
